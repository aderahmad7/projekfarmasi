            ...
            <form id="quiz-form" class="primary-form" method="POST" action="<?= base_url('posttest/submit') ?>">
                ...
                foreach ($pertanyaan as $p): ?>
                    ...
                    <div class="goal-title">
                        <p><?= esc($p['pertanyaan']) ?></p>
                    </div>
                    <div class="primary-goal-content mt-32">
                        <?php foreach ($pilihan[$p['id']] as $pil): ?>
                            <div class="form-check select-goal mt-12">
                                <input class="form-check-input custom-input-goal"
                                    name="pilihan[<?= esc($p['id']) ?>]"
                                    type="radio"
                                    id="language<?= esc($pil['id']) ?>"
                                    value="<?= esc($pil['id']) ?>" />
                                <label class="form-check-label custom-lable-goal"
                                    for="language<?= esc($pil['id']) ?>">
                                    <?= esc($pil['teks_pilihan']) ?>
                                </label>
                            </div>
                        <?php endforeach; ?>
                    </div>
                ...
                <!-- Kirim hidden input kalau mau kirim ID user atau lainnya -->
                <!-- <input type="hidden" name="id_pasien" value="<?= session()->get('id_user') ?>"> -->
